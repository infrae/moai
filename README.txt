...
MOAI has been specificly developed for universities, and contains a lot of hard-earned wisdom. The software has been in use since 2007, and a lot of features were added since that time. In late 2008, the software was completely refactored and packaged up under the name "MOAI". You can read more about this on the `MOAI History`_ page.

MOAI is a standalone system, so it can be used in combination with any repository software that comes with an OAI feed such as `Fedora Commons`_, `EPrints`_ or `DSpace`_. It can also be used directly on an SQL database or just a folder of xml files.
...
MOAI has some interesting features that are not common in most OAI servers. 
Besides serving OAI, it can also harvest OAI, this makes it possible for MOAI to work as a pipe, where the OAI data can be reconfigured, cached and enriched while it passes through the MOAI code.
